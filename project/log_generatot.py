...
"""
Randomly traverses the CFG to build a fake control flow log
Args:
    cfg - The control flow graph
    output_file - The log file name
    loops - The number of entries we want in the log
    start -  The address of the first node in the graph
"""
def generate_log(cfg, output_file, loops, start):
    ...
    for i in range(loops):
        ...
        if len(current_node.successors) == 0: # Catch for reaching the actual end of a program
            break
...
"""
Builds an argument parser and parses arguments
Returns:
...
"""
A function that prunes all deadend branches from the graph. Its a bit jank because
some of the lists generated by the cfg generator are accidentally shared. 
Args:
    cfg - The control flow graph to prune
    application - the pruning file with the nodes to remove
"""
def clean_cfg(cfg, application):
    pruning_file = open(application, "r") # I made it a file to be easier
...
def main():
    ...
    # Removing deadends allows for arbitrary sized logs that look more natural
    if cmd_args.prune is not None:
         start_addr = "0xe000"
         clean_cfg(cfg, cmd_args.prune)

    # A more random approach but often gets large amounts of repeated values when an actual sink is reached
    else:
